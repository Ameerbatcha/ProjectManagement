...
def final():
    ...
    for i in range (len(q.data)):
        out = q.data[i]
        print(out["username"],out["password"],out["mode"])
        if out["username"] == user_name and out["password"]==hash_it(password):
            view=out["mode"]
            return render_template("dashboard.html",view=view,vetha=vetha , data = hi(h = supabase.table("Projectdetails").select("*").execute()))
    else:
        return render_template("home.html",p="username or password is wrong")


@app.route('/but')
def but():
    supabase.storage().from_("tharun").upload("maxresdefault.jpg","https://i.ytimg.com/vi/4i-h-4IDUyA/maxresdefault.jpg")
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        supabase.storage().from_("tharun").upload(file.filename,os.getcwd()+file.filename)
        url = supabase.storage().from_("tharun").get_public_url(file.filename)
        print(url)
        # exit()
        string_dict = gimme_string(url)
        plag_flag , value,index ,d= deadpool.is_plag(string_dict)
        print (plag_flag , value,index,d)
        if value < 2:
            v = "2.png"
        elif value < 5:
            v="3.png"
            ...
            v="4.png"
        if plag_flag:
            supabase.storage().from_("tharun").move(file.filename, 'Plag/'+file.filename)
            ...
            labels = ["own content","Copied"]
            value = len(list(d.values())[0])
